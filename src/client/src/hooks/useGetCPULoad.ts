...
export const useGetCPULoad = (shouldMockHighLoad: boolean) => {
  ...
  const initialCPULoad = data.length ? data[data.length - 1].value + mockHighLoad : 0;
  ...
  useInterval(async () => {
    ...
    setData(points => [...points, {date: new Date(), value: dataPoint.cpuLoadAverage + mockHighLoad}].slice(sliceData));
