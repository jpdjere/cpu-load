...
describe("useAlerts hook", () => {
  ...
  describe("high cpu load data point is processed", () => {
    it("should update consecutiveHighs if a high data point is processed", () => {
      const { result } = renderHook(
        ({ defaults }) => useAlerts(OVER_LIMIT_LOAD, defaults),
        {
          initialProps: {
            defaults: {
    ...
    it("should prepend a new alert phase if the the cpu load surpasses the threshold for two minutes", () => {
      ...
      expect(result.current.alertPhases.length).toEqual(
        preexistingAlertPhases.length + 1
      );
  ...
  describe("low cpu load data point is processed", () => {
    it("should update consecutiveLows if a low data point is processed", () => {
      const { result } = renderHook(
        ({ defaults }) => useAlerts(UNDER_LIMIT_LOAD, defaults),
        {
          initialProps: {
            defaults: {
    ...
    it("should close the ongoing alert phase (with a timestamp) if the the cpu load is under the threshold for two minutes", () => {
      ...
      const preexistingAlertPhases = [
        { start: lastAlertStartTimeStamp },
